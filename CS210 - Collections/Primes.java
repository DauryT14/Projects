...
public class Primes implements Iterable<Integer> {
    ...
    private class PrimesIterator implements Iterator<Integer> {
        ...
        public boolean hasNext() {
            // returning true if the number of primes returned so far
            // are less than the total number of primes we need
            // else, otherwise
            return count < n;
        }
        ...
        public Integer next() {
            // looping while (p) is not prime
            while(!isPrime(p)){
                // we increment p since it is not prime
                p++;
            }
            count++; // incrementing the number of primes returned so far
            return p++; // returning p and then incrementing it
        }
